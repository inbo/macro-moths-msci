...
save_figure <- function(x, file, devices, dpi = 300, width = 8, height = 6) {
  sapply(devices, function(dev) {
    ggsave(paste0(file.path("media", file), ".", dev),
         x,
         dpi = dpi,
         width = width,
         height = height)
...
```{r}
sapply(devices, function(dev) {
  ggsave(paste0(file.path("media", "sci_barplot_conference."), dev),
       sci_barplot,
       dpi = 300,
       width = 8,
       height = 6)

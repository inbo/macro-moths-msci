...
library(tidyverse)
library(INBOtheme)
theme_set(theme_inbo(base_size = 10, transparent = TRUE))
```
...
```{r}
p_biotope1 <- msci_draw_list_log$Biotope %>%
  ggplot(aes(y = label)) +
...
```{r}
p_hostplantspecificity1 <- msci_draw_list_log$HostPlantSpecificity %>%
  ggplot(aes(y = label)) +
    geom_violin(aes(x = msci.draw, fill = trend_coarse),
                colour = alpha("white", 0)) +
    geom_vline(xintercept = 0, linetype = "longdash", colour = "black") +

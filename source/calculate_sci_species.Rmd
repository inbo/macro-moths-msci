...
summary_draw <- summary_draw_pre %>%
  mutate(
    trend_fine = case_when(
        ...
        effect_fine == "?+" ~ "potential increase",
        effect_fine == "?-" ~ "potential decrease",
        TRUE ~ "uncertain",
    ),
...
We also include trends of the `r nrow(extra_spec_df)` who emerged or dissapeared (zero counts).
...
organised_summary_df <- summary_draw %>%
  ...
  select(species_nl, sci.median, sci.lower, sci.upper,
         trend_coarse, trend_fine, certainty, .point, .interval, .width) %>%

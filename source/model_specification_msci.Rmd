...
fit3 <- brm(
  bf(n_grids ~ period * HostPlantSpecificity * NutrientValueBiotope + offset(log(sum_per_period)) +
    (1 + period || species_nl)),
...
msci_draws_ellenberg_phagy %>%
  full_join(summ_ellenberg_phagy, by = join_by(NutrientValueBiotope, HostPlantSpecificity)) %>%
  mutate(fill_var = case_when(
    .lower > 0 ~ "chartreuse3",
    .upper < 0 ~ "coral",
    .lower < 0 & .lower > -0.2 & .upper > 0 & .upper < 0.25 ~ "gold",
...
```{r}
msci_draws_nozero %>%
  full_join(summ_ellenberg_phagy_nozero, by = join_by(NutrientValueBiotope, HostPlantSpecificity)) %>%
  mutate(fill_var = case_when(
    .lower > 0 ~ "chartreuse3",
    .upper < 0 ~ "coral",
    .lower < 0 & .lower > -0.2 & .upper > 0 & .upper < 0.25 ~ "gold",

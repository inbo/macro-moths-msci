...
- half dist and 2 intervals vertical
...
- half dist and 2 intervals horizontal

```{r}
msci_draws_figtest %>%
  mutate(Activity = paste0(Activity, "\n", "(n = ", n_species, ")")) %>%
  ggplot(aes(y = Activity)) +
    stat_halfeye(aes(x = msci, fill = fill_var), orientation = "horizontal",
             .width = c(0.6, 0.9)) +
    geom_vline(xintercept = 0, linetype = 2) +
    geom_vline(xintercept = c(min_threshold, max_threshold), linetype = 3) +
    scale_fill_manual(values =  c("firebrick1",
                                  "chartreuse3",
                                  "gold",
                                  "skyblue"),
...
msci_draws_figtest %>%
  mutate(Activity = paste0(Activity, "\n", "(n = ", n_species, ")")) %>%
  ggplot(aes(y = Activity)) +
    stat_eye(aes(x = msci, fill = fill_var), orientation = "horizontal",
             .width = c(0.9), linewidth = 1) +
    geom_errorbar(data = summary_figtest %>%
                    mutate(Activity = paste0(Activity,
                             "\n", "(n = ", n_species, ")")),

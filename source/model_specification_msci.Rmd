...
msci_draws_figtest %>%
  ...
  ggplot(aes(y = Activity)) +
    ...
    geom_vline(xintercept = c(min_threshold, max_threshold),
               linetype = "dotdash") +
    stat_pointinterval(aes(x = msci, fill = fill_var),
...
p_final <- msci_draws_figtest %>%
  mutate(Activity = paste0(Activity, "\n", "(n = ", n_species, ")")) %>%
  ggplot(aes(y = Activity)) +
    stat_eye(aes(x = msci, fill = fill_var),
             .width = c(0.9), linewidth = 1) +
...
p_final +
  ...
  annotate("text", y = c(0.5, 0.5), x = c(0 + 0.1, max_threshold + 0.15),
           label = c("increase", "strong increase"), size = 2) +
  annotate("segment", y = c(0.6, 0.6), x = c(0 + 0.03, max_threshold + 0.03),
           xend = c(0 + 0.1, max_threshold + 0.1), yend = c(0.6, 0.6),
           arrow = arrow(type = "closed", length = unit(0.01, "npc"))) +
  annotate("text", y = c(0.5, 0.5), x = c(0 - 0.1, min_threshold - 0.15),
           label = c("decrease", "strong decrease"), size = 2) +
  annotate("segment", y = c(0.6, 0.6), x = c(0 - 0.03, min_threshold - 0.03),
           xend = c(0 - 0.1, min_threshold - 0.1), yend = c(0.6, 0.6),
           arrow = arrow(type = "closed", length = unit(0.01, "npc")))
```

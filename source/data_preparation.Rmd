...
species_traits_final_nozero_unfiltered <- species_traits_joined_nozero %>%
  pivot_longer(
    cols = Biotope:Family,
    names_to = "trait_name",
...
species_traits_final_unfiltered <- species_traits_joined %>%
  pivot_longer(
    cols = Biotope:Family,
    names_to = "trait_name",
    values_to = "trait_value",
...
```{r}
species_traits_final_nozero <- species_traits_final_nozero_unfiltered %>%
  left_join(check_list_flanders_raw %>%
...
species_traits_final <- species_traits_final_unfiltered %>%
  left_join(check_list_flanders_raw %>%
                select(species_nl, Status),
              by = join_by(species_nl)) %>%
  filter(Status == "Standvlinder" | trait_name == "Migration") %>%

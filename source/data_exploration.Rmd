...
species_traits_final_nozero <- read_csv(
  file.path(data_path, "prop_data_per_trait_nozero_intermed.csv")
)
species_traits_final <- read_csv(file.path(data_path,
                                           "prop_data_per_trait_intermed.csv"))
```
...
Multiple Correspondence Analysis ([MCA](http://www.sthda.com/english/articles/31-principal-component-methods-in-r-practical-guide/114-mca-multiple-correspondence-analysis-in-r-essentials/)) is an alternative to PCA for categorical variables. The `MCA()` function from the **FactoMineR** package can be used to implement MCA. We exclude `HabitatTemperature` and `HabitatHumidity` since `TempHum` is already included.
...
```{r}
fviz_mca_var(traits_mca, axes = c(1, 2), choice = "mca.cor", repel = TRUE,

...
Multiple Correspondence Analysis (MCA) is an alternative to PCA for categorical variables. The `MCA()` function from the **FactoMineR** package can be used to implement MCA.
...
```{r}
dat_mca <- filtered_df %>%
  filter(!trait_name %in% c("Biotope")) %>%
...
fviz_mca_biplot(traits_mca, geom.ind = "point", ggtheme = theme_minimal(), repel = TRUE,
                alpha.ind = 0, col.var = "contrib")
```

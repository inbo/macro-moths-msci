...
cross_tab <- function(df, row, column, print = TRUE) {
  ...
  require(tidyr)
  cross_table <- df %>%
    filter(trait_name %in% c(row, column)) %>%
    distinct(.data$species_nl, .data$species_new, .data$trait_name, 
             .data$trait_value) %>%
    pivot_wider(
    ...
    drop_na() %>%
    group_by(species_nl) %>%
    arrange(species_nl, species_new, !!sym(column)) %>%
    mutate("{column}" := ifelse(is.na(!!sym(column)), 

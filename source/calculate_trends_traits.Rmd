...
$$
n_{1j} \sim Poisson(\lambda_{1j})\\
...\\
n_{Sj} \sim Poisson(\lambda_{Sj})
$$

So that we can specify the model
...
fit <- brm(bf(n_grids ~ period + offset(log(sum_per_period)) + (1 + period || species_nl)), 
            data = dat, family = poisson(),
            chains = nchains, warmup = burnin, iter = niter, cores = nparallel)

plot(fit)

pp_check(fit, type = "dens_overlay_grouped", ndraws = 100, 
         group = "period")

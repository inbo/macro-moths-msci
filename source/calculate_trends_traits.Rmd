...
```{r}
dat_selection <- species_traits_final %>%
  filter(trait_name %in% c("EllenbergN", "Phagy", "Activity")) %>%
  pivot_wider(
    id_cols = c(species_nl, n_grids, period, sum_per_period),
    names_from = trait_name, values_from = trait_value
  )
...
test <- brm(
  bf(n_grids ~ period * Phagy * EllenbergN * Activity + offset(log(sum_per_period)) +
    (1 + period | species_nl)),
  data = dat_selection, family = poisson(),
  chains = nchains, warmup = burnin, iter = niter, cores = nparallel,
  thin = thinning
)

refm_poiss <- get_refmodel(test, latent = TRUE)

...
```{r trait-data-combi}
trait_combi_df_list <- lapply(trait_combinations, function(combo) {
  if (length(combo) == 2) {
    row <- combo[1]
    column <- combo[2]
    out <- species_traits_final %>%
...
names(formula_list) <- names(trait_combi_df_list)
```
...
model_msci_list <- vector("list", length = length(trait_combi_df_list))
model_msci_list_names <- vector(length = length(trait_combi_df_list))

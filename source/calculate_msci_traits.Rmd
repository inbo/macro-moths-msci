...
```{r}
set_priors_brms <- function(combo, df) {
...
for (i in seq_along(trait_df_list)) {
  ...
  # set priors
  prior_combo <- set_priors_brms(combo_name, trait_df_list[[i]])
...
for (i in seq_along(trait_df_list)) {
  ...
  if (combo_name %in% trait_combinations_to_fit) {
    ...
    obj_name <- paste0("fit_msci_", combo_name)
    print(paste("Busy loading", obj_name, "..."))
    assign(obj_name,
           brm(formula = formula_list[[i]],
...
```{r}
rhat_list2 <- get_rhats(model2_msci_list)
show_rhat(do.call(rbind.data.frame, rhat_list2))
```

```{r}
converged_models_list <- vector("list")
converged_models_list_names <- vector()
j <- 0
for (i in seq_along(rhat_list2)) {
  df <- rhat_list2[[i]]
  if (max(df$rhat) <= 1.1) {
    j <- j + 1
    converged_models_list[[j]] <- model2_msci_list[[i]]
    converged_models_list_names[j] <- names(model2_msci_list)[i]
  }
}
names(converged_models_list) <- converged_models_list_names
```

There are still models that did not converge.
This is because there are some groups that did not have any data from the beginning.

> add constant priors? https://github.com/paul-buerkner/brms/issues/783

This works. We can automate this but what about when the missing group is the reference group?




```{r}
unconverged_models <- setdiff(model2_msci_list_names, 
                              converged_models_list_names)
model3_msci_list <- vector("list", length = length(unconverged_models))

for (i in seq_along(unconverged_models)) {
  combo_name <- unconverged_models[i]
  obj_name <- paste0("fit3_msci_", combo_name)
  print(paste("Busy fitting", obj_name, "..."))
  
  # set priors of missing groups to zero
  prior_fixed <- set_priors_brms(combo_name, trait_df_list[[combo_name]])
  assign(obj_name, brm(formula = formula_list[[combo_name]],
  ...
  model3_msci_list[[i]] <- get(obj_name)
}

names(model3_msci_list) <- unconverged_models
```

```{r}
rhat_list3 <- get_rhats(model3_msci_list)
show_rhat(do.call(rbind.data.frame, rhat_list3))
```

```{r}
i = 1
combo_name <- unconverged_models[i]
dat <- trait_df_list[[combo_name]] %>%
  mutate(Biotope = relevel(factor(Biotope), "Woodland"))

# set priors of missing groups to zero
prior_fixed <- set_priors_brms(combo_name, dat)
biotope_ellenberg_test <- brm(formula = formula_list[[combo_name]],
           data = dat,
           family = poisson(),
           prior = prior_fixed,
           chains = nchains,
...
```{r}
test_rhat <- get_rhats(list(model = biotope_ellenberg_test))
show_rhat(do.call(rbind.data.frame, test_rhat))
```
...
combo_name <- unconverged_models[i]
dat <- trait_df_list[[combo_name]] %>%
  mutate(LarvalFoodSource2 = relevel(factor(LarvalFoodSource2), "Shrubs_Trees"),
         Phagy = relevel(factor(Phagy), "Polyphagous")) %>%
  filter(LarvalFoodSource2 != "Other")

# set priors of missing groups to zero
prior_fixed <- set_priors_brms(combo_name, dat)
Phagy_LarvalFoodSource2 <- brm(formula = formula_list[[combo_name]],
           data = dat,
           family = poisson(),
           prior = prior_fixed,
           chains = nchains,
...
# set priors of missing groups to zero
prior_fixed <- set_priors_brms(combo_name, dat)
LarvalFoodSource2_EllenbergN <- brm(formula = formula_list[[combo_name]],
           data = dat,
           family = poisson(),
           prior = prior_fixed,
           chains = nchains,
...
dat <- trait_df_list[[combo_name]] %>%
  mutate(Distribution = relevel(factor(Distribution), "Palaearctic"),
         TempHum = relevel(factor(TempHum), "Hot_Dry"))

# set priors of missing groups to zero
prior_fixed <- set_priors_brms(combo_name, dat)
Distribution_TempHum <- brm(formula = formula_list[[combo_name]],
           data = dat,
           family = poisson(),
           prior = prior_fixed,
           chains = nchains,
...
```{r}
test_rhat2 <- get_rhats(list("Phagy_LarvalFoodSource2" = Phagy_LarvalFoodSource2,
                            "LarvalFoodSource2_EllenbergN" = LarvalFoodSource2_EllenbergN,
                            "Distribution_TempHum" = Distribution_TempHum))
show_rhat(do.call(rbind.data.frame, test_rhat2))
```
...
```{r}
for (i in seq_along(converged_models_list)) {
  title <- paste("Fit referentiemodel", names(converged_models_list)[i])
  model <- converged_models_list[[i]]
  print(
    pp_check(
      model,
  ...
  print(
    pp_check(model,
      type = "dens_overlay_grouped", ndraws = 100,
      group = str_split_i(names(converged_models_list)[i], "_", 1)
      ) +
      ggtitle(title)
    )
  ...
  print(
    pp_check(model,
      type = "dens_overlay_grouped", ndraws = 100,
      group = str_split_i(names(converged_models_list)[i], "_", 2)
      ) +
      ggtitle(title)
    )
}

...
draws_mat_list <- lapply(model_msci_combi_list, function(model) {
  model %>%
...
contrast_mat_list <- vector("list", length = length(model_msci_combi_list))
for (i in seq_along(model_msci_combi_list)) {
  model <- model_msci_combi_list[[i]]
  name <- names(model_msci_combi_list)[i]

  contrast_mat_list[[i]] <- get_contrast_matrix(model, name)
}
names(contrast_mat_list) <- names(model_msci_combi_list)

# get combination names
names_list <- vector("list", length = length(model_msci_combi_list))
for (i in seq_along(model_msci_combi_list)) {
  model <- model_msci_combi_list[[i]]
  name <- names(model_msci_combi_list)[i]
  ...
  names_list[[i]] <- col_names
}
names(names_list) <- names(model_msci_combi_list)

# calculate msci
msci_draw_list <- lapply(names(model_msci_combi_list), function(name) {
  trait1 <- stringr::str_split_i(name, "_", 1)
  trait2 <- stringr::str_split_i(name, "_", 2)
  ...
  msci_df <- as_tibble(
    ...
    `colnames<-`(unlist(names_list[[name]])) %>%
...
names(msci_draw_list) <- names(model_msci_combi_list)

# filter groups with too few species
msci_draw_list_filtered <- lapply(msci_draw_list, function(draws) {
  draws %>%
    filter(n_species > filter_size)
})

# get summary statistics
summary_draw_list <- lapply(names(msci_draw_list_filtered), function(name) {
  trait1 <- stringr::str_split_i(name, "_", 1)
  trait2 <- stringr::str_split_i(name, "_", 2)

  msci_draws <- msci_draw_list_filtered[[name]]
...
names(summary_draw_list) <- names(model_msci_combi_list)
```
...
for (i in seq_along(summary_draw_list)) {
  ...
  draws_df <- msci_draw_list_filtered[[name]]
  trait1 <- stringr::str_split_i(name, "_", 1)
  trait2 <- stringr::str_split_i(name, "_", 2)

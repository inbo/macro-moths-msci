...
> ignore these parameters (we do not need them anyway)?

```{r}
prior_fixed <- c(
  prior(constant(0),
        class = "b",
        coef = "EllenbergNVeryNutrientPoor:TempHumHot_Wet"),
  prior(constant(0),
        class = "b",
        coef = "periodp2013_2022:EllenbergNVeryNutrientPoor:TempHumHot_Wet")
  )

test_fixed_fit <- brm(
  formula = formula_list[[12]],
  data = trait_df_list$EllenbergN_TempHum,
  family = poisson(),
  prior = prior_fixed,
  chains = nchains,
  warmup = burnin/2,
...
set_fixed_priors <- function(combo) {
  ...
  missing_combinations <- setdiff(all_combinations,
...
for (i in seq_along(unconverged_models)) {
  ...
  prior_fixed <- set_fixed_priors(combo_name)

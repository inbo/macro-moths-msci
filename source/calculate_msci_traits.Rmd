...
```{r}
draw_matrix <- model_msci_list$Phagy_EllenbergN %>%
  spread_draws(`b_periodp2013_2022.*`, regex = TRUE) %>%
  select(starts_with("b_periodp2013_2022")) %>%
  as.matrix()

contrast_mat <- get_contrast_matrix(model_msci_list$Phagy_EllenbergN,
                                    "Phagy_EllenbergN")

ellenbergn <- sort(unique(model_msci_list$Phagy_EllenbergN$data$EllenbergN))
phagy <- sort(unique(model_msci_list$Phagy_EllenbergN$data$Phagy))
col_names <- lapply(ellenbergn, function(i) {
  paste(rep(i, 3), phagy, sep = "_")
})

msci_draws <- as_tibble(

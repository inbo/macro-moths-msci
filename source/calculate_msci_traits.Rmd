...
contrast_mat_list <- vector("list", length = length(model_msci_list))
for (i in seq_along(model_msci_list)) {
  model <- model_msci_list[[i]]
  name <- names(model_msci_list)[i]
...
names_list <- vector("list", length = length(model_msci_list))
for (i in seq_along(model_msci_list)) {
  model <- model_msci_list[[i]]
  name <- names(model_msci_list)[i]
...
names(names_list) <- names(model_msci_list)
...
for (i in seq_along(summary_draw_list)) {
  ...
  p <- draws_df %>%
    ...
    mutate(
      colours = case_when(
          msci.lower > 0 ~ "chartreuse3",
          msci.upper < 0 ~ "coral",
          msci.lower < 0 & msci.lower > -0.2 &
            msci.upper > 0 & msci.upper < 0.25 ~ "gold",
          TRUE ~ "skyblue",
      ),
      fill_var = factor(colours,
        levels = c("coral", "gold", "chartreuse3", "skyblue"),
        ordered = TRUE)
    ) %>%
    mutate(
    ...
    ggplot(aes(x = .data[[trait1]])) +
      ...
      scale_fill_identity(guide = "legend",
                          labels = c("negative effect", "no effect",
                                     "positive effect", "unknown effect")) +
      labs(x = "",
           y = "Multispecies Change Index",
           fill = "",
           title = gsub("_", " ~ ", name)) +
      theme(
